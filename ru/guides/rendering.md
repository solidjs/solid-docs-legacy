...
Solid поддерживает создание шаблонов в трех формах: JSX, тегированные шаблонные литералы и вариант Solid HyperScript, хотя JSX является преобладающей формой.

Почему JSX - отличный DSL для компиляции? Он имеет четкий синтаксис, поддерживает TypeScript, работает с Babel и поддерживает другие инструменты, такие как выделение синтаксиса кода и Prettier. Было только прагматично использовать инструмент, который, по сути, дает вам все это бесплатно. В качестве скомпилированного решения он обеспечивает отличный DX. Зачем бороться с настраиваемыми синтаксическими DSL, если вы можете использовать один так широко поддерживаемый?
...
Большинство всех атрибутов в собственном элементе JSX заданы как атрибуты DOM. Статические значения встроены прямо в клонируемый шаблон. Есть ряд исключений, таких как `class`, `style`, `value`, `innerHTML`, которые обеспечивают дополнительную функциональность.

Однако настраиваемые элементы (за исключением встроенных встроенных модулей) по умолчанию имеют свойства в динамическом режиме. Это для обработки более сложных типов данных. Он выполняет это преобразование с помощью стандартных имен атрибутов регистра змейки `some-attr` в `someAttr`.

Однако можно управлять этим поведением напрямую с помощью директив пространства имен. Вы можете принудительно использовать атрибут с помощью `attr:`или prop с `prop:`
...
Самый простой способ смонтировать Solid - импортировать рендер из `solid-js/web`. `render` принимает функцию в качестве первого аргумента и контейнер для монтирования в качестве второго и возвращает метод удаления. Этот рендеринг автоматически создает реактивный корень и обрабатывает рендеринг в контейнере монтирования. Для лучшей производительности используйте элемент без дочерних элементов.
...
Поскольку все узлы JSX являются фактическими узлами DOM, единственная обязанность компонентов верхнего уровня - добавить их в DOM.
