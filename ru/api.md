...
**Будьте осторожны !!!** Вставка в `innerHTML` любых непроверенных данных, которые приходят от конечного пользователя, могут стать источником злонамеренной атаки.
...
Выражение, передаваемое в компонент или Эффект и содержащее вызов функции, доступ к свойству или JSX, заключается в геттер. Зная это, мы можем уменьшить накладные расходы на вещи, которые никогда не изменятся, просто обращаясь к ним за пределами JSX. Простая переменная никогда не будет обернута.
