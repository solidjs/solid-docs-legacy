...
정적 배열의 경우 map을 사용하는데 아무 문제가 없습니다. 하지만 시그널이나 리액티브 프로퍼티에 대해 루프를 도는 경우, `map`은 충분하지 않습니다: 배열이 어떤 이유로 변경된다면 _전체 map_ 은 다시 실행되며, 모든 노드가 다시 생성됩니다.

`<For>` 와 `<Index>`는 둘 다 이보다 더 똑똑한 루프 솔루션을 제공합니다. 각각의 렌더링된 노드는 배열의 엘리먼트에 매핑됩니다. 배열의 엘리먼트가 변경되면 매핑된 노드는 다시 렌더링됩니다.

`<Index>` 는 이 작업을 _인덱스를 사용해_ 수행하며, 각 노드는 배열의 인덱스에 연결됩니다; `<For>` 는 이 작업을 _값을 사용해_ 수행하며, 각 노드는 배열의 데이터 조각에 연결됩니다. 이 때문에 콜백에서 `<Index>`가 항목에 대한 시그널을 제공하는 이유입니다. 각 항목의 인덱스는 고정된 것으로 간주되지만 해당 인덱스의 데이터는 변경될 수 있습니다. 반면에 `<For>`는 인덱스에 대한 시그널을 제공합니다. 항목의 내용은 고정된 것으로 간주되지만 항목은 배열내에서 이동할 수 있습니다.

예를 들어, 배열의 두 요소가 바뀌면, `<For>`는 연결된 두 DOM 노드의 위치를 변경하고 그 과정에서 `index()` 시그널을 업데이트합니다. `<Index>`는 DOM 노드의 위치를 변경하지 않지만, 두 DOM 노드에 대한 `item()` 시그널을 업데이트하고 다시 렌더링하도록 합니다.

차이점에 대한 자세한 설명은 Ryan의 동영상에서 [이 부분](https://www.youtube.com/watch?v=YxroH_MXuhw&t=2164s)을 참고하세요.

### 왜 props를 디스트럭처링하게 되면 반응성을 잃게 되나요?

props 객체를 사용하면 프로퍼티 액세스를 추적하여 반응성이 활성화됩니다.
JSX 표현식이나 이펙트와 같은 _추적 범위_내에서 프로퍼티에 액세스하면, 해당 프로퍼티가 변경될 때 JSX 표현식이 다시 렌더링되거나 이펙트가 다시 실행됩니다.

디스트럭처링을 하게 되면 객체의 프로퍼티에 액세스하게 됩니다. 이 작업이 추적 범위 밖에서 일어난다면, Solid는 코드를 다시 실행할 수 없게 됩니다.

다음 예제에서는 JSX 템플릿 내에서 프로퍼티 액세스가 발생했기 때문에, 시그널 변경시 이를 추적해 span 컨텐츠를 업데이트합니다:
...
하지만 다음 예제들은 템플릿 외부에서 프로퍼티에 액세스했기 때문에 span 텍스트가 업데이트되지 않습니다:
...
### 시그널과 스토어는 언제 사용하나요? 어떤 차이가 있나요?

스토어는 중첩된 값을 자동으로 래핑므로 심층 데이터 구조와 모델과 같은 것에 적합합니다. 다른 대부분의 경우 시그널은 가볍고 훌륭하게 작업을 수행합니다.

우리가 이를 하나로 묶고 싶어도, 프리미티브를 프록시할 수 없으므로 이 경우 함수를 사용합니다. 함수는 가장 간단한 인터페이스이며 모든 리액티브 표현식(상태 액세스 포함)은 전송시 하나로 래핑될 수 있으므로 유니버설 API를 제공합니다. 시그널이나 상태에 원하는 이름을 지정할 수 있어 최소한의 기능에 머무를 수 있습니다. 최종 사용자에서 `.get()` `.set()` 입력을 강요하거나, 심지어 `.value`를 입력하게 강요하는 것은 절대로 피하고 싶은 것입니다. 적어도 전자는 간결함을 위해 alias를 지정할 수 있지만, 후자는 함수를 호출하는 매우 간결하지 않은 방법입니다.

### 왜 Vue 처럼 Solid 스토어에 값을 할당할 수 없는건가요? Svelte 나 MobX? 양방향 바인딩은 어떻게 하나요?
